...
## Returns {visual, left_arm, right_arm, left_leg, right_leg, torso}
static func build_body(host: Node3D, face_tex: Texture2D) -> Dictionary:
	...
	torso.size = Vector3(0.58, 0.72, 0.34)
	add_part(torso_root, torso, blazer_mat, Vector3(0, 1.18, 0), "Blazer")

	var lapel_l := BoxMesh.new()
	lapel_l.size = Vector3(0.12, 0.42, 0.04)
	add_part(torso_root, lapel_l, blazer_dark, Vector3(-0.14, 1.35, -0.18), "LapelL")
	var lapel_r := BoxMesh.new()
	lapel_r.size = Vector3(0.12, 0.42, 0.04)
	add_part(torso_root, lapel_r, blazer_dark, Vector3(0.14, 1.35, -0.18), "LapelR")

	var shirt := BoxMesh.new()
	shirt.size = Vector3(0.28, 0.35, 0.08)
	add_part(torso_root, shirt, shirt_mat, Vector3(0, 1.38, -0.16), "ShirtFront")
	var col_l := BoxMesh.new()
	col_l.size = Vector3(0.14, 0.06, 0.1)
	add_part(torso_root, col_l, shirt_mat, Vector3(-0.1, 1.55, -0.14), "CollarL")
	var col_r := BoxMesh.new()
	col_r.size = Vector3(0.14, 0.06, 0.1)
	add_part(torso_root, col_r, shirt_mat, Vector3(0.1, 1.55, -0.14), "CollarR")
	...
	add_part(torso_root, tie_knot, tie_mat, Vector3(0, 1.48, -0.2), "TieKnot")
	var tie_blade := BoxMesh.new()
	tie_blade.size = Vector3(0.09, 0.42, 0.04)
	add_part(torso_root, tie_blade, tie_mat, Vector3(0, 1.22, -0.2), "TieBlade")
	...
	var crest_mi := add_part(torso_root, crest, crest_mat, Vector3(0.22, 1.28, -0.19), "PrefectCrest")
	crest_mi.rotation_degrees = Vector3(90, 0, 0)

	var hem := BoxMesh.new()
	hem.size = Vector3(0.64, 0.08, 0.38)
	add_part(torso_root, hem, blazer_dark, Vector3(0, 0.8, 0), "BlazerHem")

	var head_mesh := SphereMesh.new()
	head_mesh.radius = 0.165
	head_mesh.height = 0.33
	add_part(torso_root, head_mesh, skin_mat, Vector3(0, 1.72, 0), "Head")
	...
	face_plane.size = Vector2(0.25, 0.29)
	var face_mi := add_part(torso_root, face_plane, face_mat, Vector3(0, 1.72, -0.155), "FaceCard")
	face_mi.rotation_degrees = Vector3(90, 0, 0)

	var ear := SphereMesh.new()
	ear.radius = 0.045
	ear.height = 0.08
	add_part(torso_root, ear, skin_dark_mat, Vector3(-0.17, 1.72, 0.02), "EarL", Vector3(0.6, 1.0, 0.8))
	var ear_r := SphereMesh.new()
	ear_r.radius = 0.045
	ear_r.height = 0.08
	add_part(torso_root, ear_r, skin_dark_mat, Vector3(0.17, 1.72, 0.02), "EarR", Vector3(0.6, 1.0, 0.8))

	var hair_cap := SphereMesh.new()
	hair_cap.radius = 0.185
	hair_cap.height = 0.28
	var hair_mi := add_part(torso_root, hair_cap, hair_mat, Vector3(0, 1.82, 0.02), "HairCap")
	...
	for i in range(6):
		...
		add_part(torso_root, nub, hair_mat, Vector3(cos(a) * 0.1, 1.88 + sin(a * 2.0) * 0.02, 0.04 + sin(a) * 0.08), "HairNub%d" % i)

	var beard := SphereMesh.new()
	beard.radius = 0.12
	beard.height = 0.14
	var beard_mi := add_part(torso_root, beard, hair_mat, Vector3(0, 1.58, -0.1), "Beard")
	beard_mi.scale = Vector3(1.05, 0.55, 0.75)
	var stash := BoxMesh.new()
	stash.size = Vector3(0.12, 0.035, 0.04)
	add_part(torso_root, stash, hair_mat, Vector3(0, 1.64, -0.17), "Mustache")

	var neck := CylinderMesh.new()
	neck.top_radius = 0.07
	neck.bottom_radius = 0.09
	neck.height = 0.12
	add_part(torso_root, neck, skin_mat, Vector3(0, 1.58, 0), "Neck")

	var left_arm := Node3D.new()
	left_arm.name = "LeftArm"
	left_arm.position = Vector3(-0.38, 1.40, 0)
	torso_root.add_child(left_arm)
	var la := CapsuleMesh.new()
	la.radius = 0.075
	la.height = 0.62
	add_part(left_arm, la, blazer_mat, Vector3(0, -0.32, 0), "LSleeve")
	var lh := SphereMesh.new()
	lh.radius = 0.065
	add_part(left_arm, lh, skin_mat, Vector3(0, -0.62, 0), "LHand")

	var right_arm := Node3D.new()
	right_arm.name = "RightArm"
	right_arm.position = Vector3(0.38, 1.40, 0)
	torso_root.add_child(right_arm)
	var ra := CapsuleMesh.new()
	ra.radius = 0.075
	ra.height = 0.62
	add_part(right_arm, ra, blazer_mat, Vector3(0, -0.32, 0), "RSleeve")
	var rh := SphereMesh.new()
	rh.radius = 0.065
	add_part(right_arm, rh, skin_mat, Vector3(0, -0.62, 0), "RHand")

	var left_leg := Node3D.new()
	left_leg.name = "LeftLeg"
	left_leg.position = Vector3(-0.13, 0.76, 0)
	visual.add_child(left_leg)
	var ll := CapsuleMesh.new()
	ll.radius = 0.09
	ll.height = 0.78
	add_part(left_leg, ll, pants_mat, Vector3(0, -0.38, 0), "LTrouser")
	var lb := BoxMesh.new()
	lb.size = Vector3(0.16, 0.09, 0.28)
	add_part(left_leg, lb, shoe_mat, Vector3(0, -0.78, -0.04), "LShoe")

	var right_leg := Node3D.new()
	right_leg.name = "RightLeg"
	right_leg.position = Vector3(0.13, 0.76, 0)
	visual.add_child(right_leg)
	var rl := CapsuleMesh.new()
	rl.radius = 0.09
	rl.height = 0.78
	add_part(right_leg, rl, pants_mat, Vector3(0, -0.38, 0), "RTrouser")
	var rb := BoxMesh.new()
	rb.size = Vector3(0.16, 0.09, 0.28)
	add_part(right_leg, rb, shoe_mat, Vector3(0, -0.78, -0.04), "RShoe")
